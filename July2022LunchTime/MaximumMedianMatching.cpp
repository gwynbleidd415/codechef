...
void inputOutput()
{
  ...
  REP(i, n) { cin >> A[i]; }
  REP(i, n) { cin >> B[i]; }
  int mid{(n + 1) >> 1}, mid1{mid - 1};
  partial_sort(A.begin(), A.begin() + mid, A.end(), greater<int>());
  partial_sort(B.begin(), B.begin() + mid, B.end(), greater<int>());
  LL minn{INT64_MAX}, temp{};
  REP(i, mid)
  {
    temp = (LL)A[i] + B[mid1 - i];
    minn = min(minn, temp);
  }
  cout << minn << '\n';
}
